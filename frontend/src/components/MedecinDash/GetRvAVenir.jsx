...
// function GetRvAVenir({ onCountChange }) {
...
//           `${API_BASE_URL}/api/Medecin/rendezVous/avenir`,
//           {
//             headers: {
...
import "../GetAllRendezVous/rdv-cards-today.css";
import "bootstrap-icons/font/bootstrap-icons.css";
// (Laisse CSS/JS Bootstrap chargés une seule fois dans App.jsx si tu veux)
import "bootstrap/dist/css/bootstrap.min.css";
import "bootstrap/dist/js/bootstrap.bundle.min.js";
import { Dropdown } from "bootstrap";
...
function GetRvAVenir({ onCountChange }) {
  ...
  // Statuts disponibles (à adapter à ton enum côté backend)
  const STATUS_OPTIONS = [
    { value: "Terminé", label: "Terminé", badge: "bg-success text-light" },
  ];

  const handleChangeStatus = async (id, newStatus, dropdownDomId) => {
    const token = localStorage.getItem("token");
    if (!token) return navigate("/login");
    ...
    try {
      ...
      setUpdatingId(null);
      // Ferme le menu via l'API Bootstrap
      const btn = document.getElementById(dropdownDomId);
      if (btn) Dropdown.getInstance(btn)?.hide();
    }
  };

  // Toggle programmatique du dropdown
  const toggleStatusDropdown = (domId) => (e) => {
    e.preventDefault();
    const btn = document.getElementById(domId);
    if (btn) Dropdown.getOrCreateInstance(btn).toggle();
  };
            ...
            <div className="rdv-scroll">
              <div className="d-flex flex-column gap-3">
                {rendezVous.map((rv, idx) => {
                  const { month, day, year, time } = fmtParts(rv);
                  const domId = `statusToggle-${rv.id ?? idx}`;
                  return (
                    <div
                      key={
                        rv.id ?? `${rv.date}-${rv.heure}-${patientLabel(rv)}`
                      ...
                      <div className="d-flex align-items-center justify-content-md-end gap-2 mt-3 mt-md-0">
                        <div className="dropdown">
                          <button
                            ...
                            type="button"
                            aria-expanded="false"
                            onClick={toggleStatusDropdown(domId)}
                            disabled={updatingId === rv.id}
                          >
                            {updatingId === rv.id
                              ? "Mise à jour..."
